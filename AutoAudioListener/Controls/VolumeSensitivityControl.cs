...
namespace AutoAudioListener.Controls {
    public partial class VolumeSensitivityControl : UserControl {
        ...
        private void UpdateTrackPosition() {
            ...
            sensitivityTrackBar.Value = (int)Math.Round(Math.Min(Math.Max(position, 0), 1) * sensitivityTrackBar.Maximum);
        }

        private void sensitivityTrackBar_ValueChanged(object sender, EventArgs e) {
            if (!ManualUpdated) {
                Value = ((float)sensitivityTrackBar.Value / sensitivityTrackBar.Maximum) * (UpperRange - LowerRange) + LowerRange;
                ValueChanged?.Invoke(this, EventArgs.Empty);
            } else {
                ManualUpdated = false;
            }
            sensitivityBar.Threshold = (int)Math.Round((float)sensitivityTrackBar.Value / sensitivityTrackBar.Maximum * sensitivityBar.Maximum);
        }

        private void updateTimer_Tick(object sender, EventArgs e) {
            double position = (Math.Min(1, MonitoringDevice.AudioMeterInformation.MasterPeakValue) - LowerRange) / (UpperRange - LowerRange);
